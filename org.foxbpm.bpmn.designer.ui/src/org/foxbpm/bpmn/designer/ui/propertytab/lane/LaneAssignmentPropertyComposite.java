...
public class LaneAssignmentPropertyComposite extends AbstractFoxBPMComposite {
	private Tree tree;
	...
	public LaneAssignmentPropertyComposite(Composite parent, int style) {
		super(parent, style);
	}
	...
	private void addConnectorInstance(final ConnectorInstance connectorInstance) {
		...
		editingDomain.getCommandStack().execute(new RecordingCommand(editingDomain) {
			...
			protected void doExecute() {
				Lane lane = (Lane) getBusinessObject();
				
				if (lane.getExtensionValues().size()>0) {
					for (ExtensionAttributeValue extensionAttributeValue : lane.getExtensionValues()) {
						FeatureMap extensionElements = extensionAttributeValue.getValue();
						potentialOwner = (PotentialOwner) extensionElements.get(FoxBPMPackage.Literals.DOCUMENT_ROOT__POTENTIAL_OWNER, true);
						ConnectorInstanceElements connectorInstanceElements = potentialOwner.getConnectorInstanceElements().get(0);
						...
						potentialOwner.getConnectorInstanceElements().add(connectorInstanceElements);
					}
				} else {
					ExtensionAttributeValue extensionElement = Bpmn2Factory.eINSTANCE.createExtensionAttributeValue();
					PotentialOwner potentialOwner = FoxBPMFactory.eINSTANCE.createPotentialOwner();
					ConnectorInstanceElements connectorInstanceElements = FoxBPMFactory.eINSTANCE.createConnectorInstanceElements();
					connectorInstanceElements.setConnrctorType("actorConnector");
					connectorInstanceElements.getConnectorInstance().add(connectorInstance);
					potentialOwner.getConnectorInstanceElements().add(connectorInstanceElements);
					
					FeatureMap.Entry extensionElementEntry = new SimpleFeatureMapEntry((org.eclipse.emf.ecore.EStructuralFeature.Internal) FoxBPMPackage.Literals.DOCUMENT_ROOT__POTENTIAL_OWNER,
							potentialOwner);
		...
		editingDomain.getCommandStack().execute(new RecordingCommand(editingDomain) {
			...
			protected void doExecute() {
				...
				if (lane.getExtensionValues().size() > 0) {
					...
					for (ExtensionAttributeValue extensionAttributeValue : lane.getExtensionValues()) {
						FeatureMap extensionElements = extensionAttributeValue.getValue();
						PotentialOwner potentialOwner = (PotentialOwner) extensionElements.get(FoxBPMPackage.Literals.DOCUMENT_ROOT__POTENTIAL_OWNER, true);
						ConnectorInstanceElements connectorInstanceElements = potentialOwner.getConnectorInstanceElements().get(0);
			...
			protected void doExecute() {
				...
				if (lane.getExtensionValues().size()>0) {
					for (ExtensionAttributeValue extensionAttributeValue : lane.getExtensionValues()) {
						...
						connectorInstanceElements.setConnrctorType("actorConnector");
						connectorInstanceElements.getConnectorInstance().add(connectorInstance);
						potentialOwner.getConnectorInstanceElements().add(index, connectorInstanceElements);
					}
				} else {
					ExtensionAttributeValue extensionElement = Bpmn2Factory.eINSTANCE.createExtensionAttributeValue();
					PotentialOwner potentialOwner = FoxBPMFactory.eINSTANCE.createPotentialOwner();
					...
					FeatureMap.Entry extensionElementEntry = new SimpleFeatureMapEntry((org.eclipse.emf.ecore.EStructuralFeature.Internal) FoxBPMPackage.Literals.DOCUMENT_ROOT__POTENTIAL_OWNER,
							potentialOwner);
					extensionElement.getValue().add(extensionElementEntry);
					lane.getExtensionValues().add(index, extensionElement);
				}
			}
		});
	}
	...
	private List<ConnectorInstance> getConnectorInstances() {
		...
		if (lane.getExtensionValues().size() > 0) {
			for (ExtensionAttributeValue extensionAttributeValue : lane.getExtensionValues()) {
				FeatureMap extensionElements = extensionAttributeValue.getValue();
				potentialOwner = (PotentialOwner) extensionElements.get(FoxBPMPackage.Literals.DOCUMENT_ROOT__POTENTIAL_OWNER, true);
				if (potentialOwner != null && potentialOwner.getConnectorInstanceElements().size()>0) {
					ConnectorInstanceElements connectorInstanceElements = (ConnectorInstanceElements) potentialOwner.getConnectorInstanceElements().get(0);
