...
public class LifeCycleWidget extends FigureCanvas {
	...
	private void createLifecycleImage() {
		if (type == PROCESS_LIFE_CYCLE) {
			imageLifeCycle = new Image(null, ImageUtil.getImageDataFromName("/connector/process_lifecycle.png")); //$NON-NLS-1$
		} else if (type == USERTASK_LIFE_CYCLE) {
			imageLifeCycle = new Image(null, ImageUtil.getImageDataFromName("/connector/usertask_lifecycle.png")); //$NON-NLS-1$
		} else if (type == OTHER_LIFE_CYCLE) {
			imageLifeCycle = new Image(null, ImageUtil.getImageDataFromName("/connector/other_lifecycle.png")); //$NON-NLS-1$
		}
	}
