...
public class ModifyNewOutputConnectorWizardPage extends WizardPage {

	private EList<Output> outputs;
	 ...
	 * @param pageName
	 */
	public ModifyNewOutputConnectorWizardPage(String pageName) {
		super(pageName);
	}
	 ...
	 * @param titleImage
	 */
	public ModifyNewOutputConnectorWizardPage(EObject be, String pageName, String title, String desc, ImageDescriptor titleImage, EList<Output> outputs, ConnectorInstance connectorInstance) {
	...
	public void createControl(Composite parent) {
		...
		// 读取所有output参数
		EList<ConnectorParameterOutput> parameterOutputs = connectorInstance.getConnectorParameterOutputs();

		if (parameterOutputs != null && parameterOutputs.size() > 0) {
			for (int i = 0; i < parameterOutputs.size(); i++) {
				final ConnectorParameterOutput connectorParameterOutput = parameterOutputs.get(i);
				...
				for (int j = 0; j < outputs.size(); j++) {
					Output parameter = outputs.get(j);
...
//								"" : connectorParameterOutput.getExpression().getValue());
				...
				outputCombo.addListener(SWT.Selection, new Listener() {
					...
					public void handleEvent(Event event) {
						/*
						 * if(outputCombo.getSelectionIndex() ==
						 * outputCombo.getItemCount()-1){ ExpressionDialog
						 * expressionDialog = new ExpressionDialog(getShell(),
						 * connectorParameterOutput .getExpression() == null ?
						 * "" :
						 * connectorParameterOutput.getExpression().getValue
						 * ()); expressionDialog.setBlockOnOpen(true);
						 * if(expressionDialog !=null && expressionDialog.open()
						 * == InputDialog.OK){
						 * outputCombo.setText(expressionDialog
						 * .getExpression()); } else{
						 * outputCombo.setText(expressionDialog
						 * .getExpression()); } }
						 */
				...
				// 创建combo
				Combo targetCombo = new Combo(gridComposite, SWT.BORDER | SWT.READ_ONLY);
				...
				targetCombo.setText(connectorParameterOutput.getVariableTarget() == null ? "" : connectorParameterOutput.getVariableTarget());
...
//					ExpressionDialog expressionDialog = new ExpressionDialog(be, getShell());
//					expressionDialog.setBlockOnOpen(true);
...
//					}
	...
	public String[] getOutputTargets() {
		...
		for (ExtensionAttributeValue eav : BpmnModelUtil.getProcessByEobj(be).getExtensionValues()) {
			org.eclipse.emf.ecore.util.FeatureMap extensionElements = eav.getValue();
		...
		// 私有
		if (!(be instanceof org.eclipse.bpmn2.Process) || !(be instanceof org.eclipse.bpmn2.impl.ProcessImpl)) {
			...
			if (baseElement.getExtensionValues().size() > 0) {
				for (ExtensionAttributeValue extensionAttributeValue : baseElement.getExtensionValues()) {
					org.eclipse.emf.ecore.util.FeatureMap extensionElements = extensionAttributeValue.getValue();
					Object objectElement = extensionElements.get(FoxBPMPackage.Literals.DOCUMENT_ROOT__DATA_VARIABLE, true);
					if (objectElement != null) {
