...
public class ModifyNewConnectorWizard extends Wizard {

	private ConnectorInstance connectorInstance;

	private ModifyRenameConnectorWizardPage modifyRenameConnectorWizardPage;

	private ModifyNewOutputConnectorWizardPage modifyOutputConnectorWizardPage;
	 ...
	 * 
	 */
	public ModifyNewConnectorWizard(EObject be, BPMN2Editor bpmn2Editor, ConnectorInstance connectorInstance) {
		this.be = be;
	...
	public void addPages() {
		// 实例化向导页面
		modifyRenameConnectorWizardPage = new ModifyRenameConnectorWizardPage(be, "modifyRenameConnectorWizardPage", "给连接器命名", null, connectorInstance);
	...
	private ConnectorInstance save() {
		...
		// 跳过策略
		Expression skipexpression = modifyRenameConnectorWizardPage.getFoxBPMExpViewer().getExpression();
		...
		// 增加跳过策略
		if(skipComment.getExpression()!=null&&!skipComment.getExpression().getValue().equals("")){
		...
		if(isTimeExecute){
			...
			Expression expressionTime=modifyRenameConnectorWizardPage.getTimeBpmExpViewer().getExpression();
			...
			Expression expressionTimeSkip=modifyRenameConnectorWizardPage.getSkipBpmExpViewer().getExpression();
