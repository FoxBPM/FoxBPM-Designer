...
//					expText.addModifyListener(null);
				...
				// 暂时还是跟text一样，保留
				else if (type.equals(types[1])) {foxBPMExpViewer = new FoxBPMExpViewer(proGroup, SWT.NONE, be);
...
//				expText.addModifyListener(null);
				...
				/*
				 * if(type.equals(types[0])) { //创建文本框 final Text
				 * connectNameText = new Text(proGroup, SWT.BORDER);
				 * connectNameText.setLayoutData(new
				 * GridData(GridData.FILL_HORIZONTAL)); //布局
				 * connectNameText.setText(expression == null ? "" :
				 * expression); if(isRequired) {
				 * isRequiredControl.add(connectNameText); }
				 * 
				 * connectNameText.addModifyListener(new ModifyListener() {
				 * 
				 * public void modifyText(ModifyEvent e) { //验证输入内容
				 * if(isRequired) { dataChange(); } } });
				 * controls.add(connectNameText); } else
				 * if(type.equals(types[1])) { //创建文本框 final Text
				 * connectNameText = new Text(proGroup, SWT.BORDER | SWT.WRAP);
				 * // 修改了是textarea时的布局 GridData gd_desctext = new
