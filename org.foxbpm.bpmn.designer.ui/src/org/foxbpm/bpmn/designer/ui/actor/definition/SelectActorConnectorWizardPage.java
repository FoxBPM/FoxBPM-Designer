...
public class SelectActorConnectorWizardPage extends WizardPage {
	...
	private EditActorConnectorWizard editConnectorWizard;
	 ...
	 * Create the wizard.
	 */
	public SelectActorConnectorWizardPage(EditActorConnectorWizard editActorConnectorWizard) {
		super("wizardPage");
		setTitle("选择处理者选择器");
		setDescription("选择你想编辑的处理者选择器");
		filter = new ConnectorFilter();
		this.editConnectorWizard = editActorConnectorWizard;
	}
...
//					File file = new File(ConnectorUtil.getConnectorPathById(connector.getConnectorId()));
					...
					MessageDialog.openInformation(null, "提示", "处理者选择器删除成功");
					
					//刷新树
					elements = (List<ITreeElement>) TreeViewerNewFactory.reloadActorTree();
					...
					boolean b = MessageDialog.openConfirm(null, "警告", "你确认要删除此目录及此目录下的所有目录和所有处理者选择器吗？");
					if (!b)
						return;
		...
		treeViewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if(element != null && !element.getRealPath().equals("")) {
					...
					editConnectorWizard.getchChooseFlowConnectorFileToEditWizardPage().getCheckboxTreeViewer().setInput(editConnectorWizard.getchChooseFlowConnectorFileToEditWizardPage().getFilesInConnectorPath());
